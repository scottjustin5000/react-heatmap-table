...
const [suppressAxis, setSuppressAxis] = useState(false)
...
const setDimensionData = ({yLabels, xLabels, width, height, useAvg }) => {
  ...
  if(width === componentWidth && height === componentHeight) return
    ...
    //setDimensionData(h, w, props.useAvg)
    // console.log('HERE', width, height)
    // console.log('THERE', h,w)
    const avg = (h + w) / 2
    if(avg < MIN) {
      setShowLabels(false)
    } 
  ...
  //setDimensionData(height, width, props.useAvg)

  //og
  // const avg = (height + width) / 2
  // if(avg < MIN) {
  //   setShowLabels(false)
  // } 
  // if(useAvg) {
...
useEffect(() => {
  ...
  // const adjusted = (componentWidth - yw)
  // const height = componentHeight / (props.yLabels.length + 1)
  // const width = adjusted / (props.xLabels.length)
  // heatHeight = height
  // heatWidth = width
  // setDimensionData(height, width, props.useAvg)
  setData(data)
...
useEffect(() => {
  ...
  const debouncedResizeHandler = debounce(() => {
    ...
    //setDimensionData({ xLabels: props.xLabels, yLabels: props.yLabels, height, width, useAvg: props.useAvg})
    // const yw = determineWidth(props.yLabels)
    // setYWidth(yw)
    // const componentWidth = ref.current.parentNode.clientWidth 
    // const componentHeight = ref.current.parentNode.clientHeight
    // setComponentWidth(componentWidth)
    // setComponentHeight(componentHeight)
    // const adjusted = (componentWidth - yw)
    // const h = componentHeight / (props.yLabels.length + 1)
    // const w = (adjusted) / (props.xLabels.length)
    // if(h !== heatHeight || w !== heatWidth) {
    //   heatHeight = h
    //   heatWidth = w
    //   setDimensionData(h, w, props.useAvg)
    // }
      ...
      <div style={{display: 'flex'}}> 
        <Tooltip key={`yatt_${i}`} title={row.label}>
            <Axis 
              ...
              color={nullColor}>{ !suppressAxis && <AxisLabel width={yWidth}>{ row.label }</AxisLabel> }
            </Axis>
  ...
  const tiles = () => props.xLabels.map(h => 
    <Tooltip key={`xatt_${h}`} title={h}>
      <XAxis key={`footer_${h}`} height={height} width={width} axisLabelColor={axisLabelColor} color={nullColor}> 
        { !suppressAxis && <AxisLabel width={width}>{ h }</AxisLabel> }
      </XAxis> 
    </Tooltip>
    )
